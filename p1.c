...
processes[j] = processes[i]; processes[i] = temp; 
} 
} 
printf("Executing process %d form time %d to %d\n", processes[i].process_id, processes[i].arrival_time,processes[i].arrival_time + processes[i].burst_time); 
} 
} 
...
int num_processes = sizeof(processes) / sizeof(processes[0]); printf("\nRound Robin:\n"); 
